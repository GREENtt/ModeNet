...
class CamVid(torch.utils.data.Dataset):
    def __init__(self, image_path, label_path, csv_path,
                 scale, loss='dice', mode='train'):
        super().__init__()
        self.mode = mode
        self.image_list = []
    ...
    def __getitem__(self, index):
        ...
        img = np.array(img)  ###(512, 640, 4)
        # load label
        label = Image.open(self.label_list[index]).convert('RGB')   #### RGBA --> RGB
        ...
        label = np.array(label)  # H,W,C   ###(512, 640, 3)  (512, 640, 3)
        ...
        if self.mode == 'train':
            ...
            label = seq_det.augment_image(label)   ###(512, 640, 3)

        # image -> [C, H, W]
        img = Image.fromarray(img).convert('RGB')  #### RGBA --> RGB
        img = self.to_tensor(img).float()   ####torch.Size([3, 512, 640])

        if self.loss == 'dice':
            # label -> [num_classes, H, W]
            label = one_hot_it_v11_dice(label, self.label_info).astype(np.uint8)   ###label:(512, 640, 3)

            label = np.transpose(label, [2, 0, 1]).astype(np.float32)  ####(4, 512, 640)
            # label = label.astype(np.float32)
            label = torch.from_numpy(label)  ###torch.Size([4, 512, 640])#############

        elif self.loss == 'crossentropy':
            label = one_hot_it_v11(label, self.label_info).astype(np.uint8)
            # label = label.astype(np.float32)
            label = torch.from_numpy(label).long()  ###torch.Size([512, 640])

        elif self.loss == 'focalloss':
            # label -> [num_classes, H, W]
            label = one_hot_it_v11(label, self.label_info).astype(np.uint8)  ###(512, 640)
            # label = label.astype(np.float32)
            label = torch.from_numpy(label).long()  ###torch.Size([512, 640])
        # print(img.size(), label.size())

        return img, label  #torch.Size([3, 640, 512]) torch.Size([640, 512])
...
if __name__ == '__main__':
    ...
    data = CamVid(['D:/Code/Data/UAVdata/train', 'D:D:/Code/Data/UAVdata/val'],
                  ['D:/Code/Data/UAVdata/train_labels', 'D:/Code/Data/UAVdata/val_labels'], 'D:/Code/Data/GREENTdata/class_dict.csv',
                  (512, 640), loss='crossentropy', mode='val')
    from model.build_BiSeNet import BiSeNet
    from utils import reverse_one_hot, get_label_info, colour_code_segmentation, compute_global_accuracy

    label_info = get_label_info('D:/Code/Data/UAVdata/class_dict.csv')
