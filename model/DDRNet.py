...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)   ###torch.Size([4, 64, 160, 128])

        x = self.layer1(x)   #torch.Size([4, 64, 160, 128])
        layers.append(x)

        x = self.layer2(self.relu(x))   ###torch.Size([4, 128, 80, 64])
        layers.append(x)

        x = self.layer3_1(self.relu(x))   ###torch.Size([4, 256, 40, 32])
        layers.append(x)
        x_ = self.layer3_1_(self.relu(layers[1]))   ###torch.Size([4, 128, 80, 64])
        x = x + self.down3_1(self.relu(x_))   ###torch.Size([4, 256, 40, 32])
        x_ = x_ + F.interpolate(
            self.compression3_1(self.relu(layers[2])),
            size=[height_output, width_output],
            mode='bilinear')  ###torch.Size([4, 128, 80, 64])

        x = self.layer3_2(self.relu(x))  ###torch.Size([4, 256, 40, 32])
        layers.append(x)
        x_ = self.layer3_2_(self.relu(x_))   ###torch.Size([4, 128, 80, 64])
        x = x + self.down3_2(self.relu(x_))   ###torch.Size([4, 256, 40, 32])
        x_ = x_ + F.interpolate(
            self.compression3_2(self.relu(layers[3])),
            size=[height_output, width_output],
            mode='bilinear')   ###torch.Size([4, 128, 80, 64])
        ...
        x = self.layer4(self.relu(x))   ###torch.Size([4, 512, 20, 16])
        layers.append(x)
        x_ = self.layer4_(self.relu(x_))   ###torch.Size([4, 128, 80, 64])
        x = x + self.down4(self.relu(x_))   ###torch.Size([4, 512, 20, 16])
        x_ = x_ + F.interpolate(
            self.compression4(self.relu(layers[4])),
            size=[height_output, width_output],
            mode='bilinear')

        x_ = self.layer5_(self.relu(x_))   ###torch.Size([4, 256, 80, 64])
        x = F.interpolate(
            self.spp(self.layer5(self.relu(x))),
            size=[height_output, width_output],
            mode='bilinear')   ###torch.Size([4, 256, 80, 64])

        x_ = self.final_layer(x + x_)   ###torch.Size([4, 3, 80, 64])

        x_ = F.interpolate(x_, size=[H, W], mode='bilinear')   ###torch.Size([4, 3, 512, 640])
