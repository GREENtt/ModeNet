...
class SSFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        APF1, cls1 = self.apf1(CFGB, x5)   ###torch.Size([4, 256, 16, 20]) torch.Size([4, 3, 16, 20])
...
class PyrmidFusionNet(nn.Module):
    ...
    def forward(self, x_high, x_low):
        ...
        lat_low = self.lateral_low(x_low)   ###torch.Size([4, 64, 128, 160])
        high_up1 = F.interpolate(x_high, size=lat_low.size()[2:], mode='bilinear', align_corners=False)   ###torch.Size([4, 64, 128, 160])
        concate = torch.cat([lat_low, high_up1], 1)   ###torch.Size([4, 128, 128, 160])
        ...
        sa = self.sa(concate)  ###torch.Size([4, 32, 128, 160])
        ca = self.ca(concate)  ###torch.Size([4, 32, 128, 160])
        # print('@@@@', sa.size(),ca.size())
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        mxpool = F.conv2d(mxpool, self.conv_sh.weight, padding=0, dilation=1) ###torch.Size([4, 64, 1, 80]) 4 64
        ...
        attt1 = att[:, 0, :, :].unsqueeze(1)  ###torch.Size([4, 1, 1, 160])
        attt2 = att[:, 1, :, :].unsqueeze(1)  ###torch.Size([4, 1, 1, 160])

        fusion = attt1 * avgpool + attt2 * mxpool  ###torch.Size([4, 64, 1, 80])
        out = F.dropout(self.fuse(fusion), p=self.drop, training=self.training) ###torch.Size([4, 64, 1, 80])

        # out = out.expand(residual.shape[0],residual.shape[1],residual.shape[2],residual.shape[3])
        out = F.relu(self.gamma * out + (1 - self.gamma) * x)  ###torch.Size([4, 64, 1, 80])
        return out
