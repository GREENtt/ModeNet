    ...
    with torch.no_grad():
        ...
        for i, (data, label) in enumerate(dataloader):
            ...
            if torch.cuda.is_available() and args.use_gpu:
                data = data.cuda()
                label = label.cuda()
            predict = model(data)[0].squeeze()  ### torch.Size([3, 512, 640])
...
def main(params):
    ...
    dataset = CamVid(test_path, test_label_path,  csv_path,
        scale=(args.crop_height, args.crop_width), mode='test')
    ...
    # args.checkpoint_path = os.path.join(args.checkpoint_path, args.model+'_'+args.optimizer+'_latest_dice_loss.pth')
    args.checkpoint_path = os.path.join(args.checkpoint_path,
                                        args.model + '_best_loss.pth')
...
if __name__ == '__main__':
    params = [
        ...
        '--model', "BiSeNetV2",  # LMFFNet、BiSeNet、STDCNet、SSFPN
